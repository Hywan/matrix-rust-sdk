...
mod tests {
    ...
    async fn modifying_timeline_limit() -> anyhow::Result<()> {
        ...
        let (timeline, mut timeline_stream) = {
            ...
            assert_matches!(timeline_stream.next().await.unwrap(), VecDiff::Replace { values } => {
        ...
        // Sync to receive messages with a `timeline_limit` set to 20.
        {
            ...
            // $X8U7Zrsndy8dmmyOyZrgFMWnGsTwoNgDMr0uMpkz2vY
            // $X8U7Zrsndy8dmmyOyZrgFMWnGsTwoNgDMr0uMpkz2vY

            dbg!(timeline_stream.next().await);
            dbg!(timeline_stream.next().await);
            dbg!(timeline_stream.next().await);
            dbg!(timeline_stream.next().await);
            dbg!(timeline_stream.next().await);
            ...
            // $iuqWeW33SbRF3lJ1my92e34H1cRdO2grDFKMYmXwd-k
        }
