...
mod tests {
    ...
    struct TestStore {
        leases: Arc<RwLock<HashMap<String, (String, Instant, LockGeneration)>>>,
    }
...
pub mod memory_store_helper {
    ...
    ///
    /// Return `Some` if the lock has been acquired (or extended). It contains the generation number.
    pub fn try_take_leased_lock(
        leases: &RwLock<HashMap<String, (String, Instant, LockGeneration)>>,
        lease_duration_ms: u32,
        key: &str,
        holder: &str,
    ) -> Option<LockGeneration> {
