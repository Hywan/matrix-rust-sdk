...
pub use matrix_sdk_base::latest_event::{LatestEventContent, LatestEventValue};
use ruma::{
    events::{
                    ...
                    match content.deserialize() {
                        Ok(content) => {
                            if let Some(content) = extract_content_from_any_message_like(content) {
                                let value = LatestEventValue::LocalIsSending(content);
...
mod tests_latest_event_content {
    ...
    use super::{find_and_map_timeline_event, LatestEventContent};
...
mod tests_latest_event_values_for_local_events {
    use assert_matches::assert_matches;
    use ruma::{events::room::message::RoomMessageEventContent, OwnedTransactionId};

    use super::{LatestEventContent, LatestEventValue, LatestEventValuesForLocalEvents};
...
mod tests_latest_event_value_non_wasm {
    ...
    use super::{
        LatestEvent, LatestEventContent, LatestEventValue, LatestEventValueBuilder,
        LatestEventValuesForLocalEvents, RoomEventCache, RoomSendQueueUpdate,
    };
    use crate::{
        client::WeakClient,
        room::WeakRoom,
